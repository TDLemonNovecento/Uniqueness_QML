...
for i in range(4):
    yvalues.append([np.asarray(dZ_slot1_list[i]), "dZ%i D%i" %(i + 1, i+1)])
    yvalues.append([np.asarray(dZ_slot2_list[i]), "dZ%i D%i" %(i + 1, i+1)])
    yvalues.append([np.asarray(dZ_slot3_list[i]), "dZ%i D%i" %(i + 1, i+1)])
    yvalues.append([np.asarray(dZ_slot4_list[i]), "dZ%i D%i" %(i + 1, i+1)])

#add error of EVCM list compared to reference to lineplots
linevalues = [[np.asarray(EVCM_aberration), "MSE to Reference"]]


pltder.plot_ethyne(indexes, yvalues, lineplots = linevalues)
